...
function addGamesToPage(games) {
    ...
    for(const game of games)
    {
        ...
        gameCard.innerHTML = `<p> Title: ${game.name} </p><p>${game.description}</p> <img src = "${game.img}" class = "game-img" />`;
...
const stringUnfund = `A total of $${pledged.toLocaleString('us-en')} has been pledged. Currently, ${unfundedNum === 0 ? "no games are unfunded." : `${unfundedNum} games are unfunded. Please help to fund these games!`}`;

// create a new DOM element containing the template string and append it to the description container
const unfundDOM = document.createElement("p");
unfundDOM.innerHTML = stringUnfund;

descriptionContainer.appendChild(unfundDOM);
